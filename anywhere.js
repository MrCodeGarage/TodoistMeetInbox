...
/*
    if(top.location.pathname.indexOf('/u/') == -1)
        return;*/
	...
	top_frame.style.backgroundColor = "rgb(66, 133, 244)";
	...
	function placeWindow() {
		...
		holder.style.left = (win_size.w - holder.offsetWidth - 350) + 'px';
    ...
    function locationPasser() {
        // Try to parse side pane first
        try {
            var cls = document.getElementsByClassName("ii gt")[0].getAttribute('class');
            var mId = cls.match(/ m([a-zA-Z0-9]+) /)[1];
            var cur_loc = location.origin + location.pathname + '#inbox/' + mId;
            var title = document.getElementsByClassName("hP")[0].innerText;
        }
        catch(e) {
            var title = top.document.title;
            var cur_loc = top.location.toString();
        }
   ...
   setInterval(locationPasser, 200);
    ...
    // Quick add
    TD_OPTS.addAsTask = function(Titel) {
      ...
      //  subject
        ...
        console.log(top.location.toString());
        var msg = {
            'msg_type': 'GMAIL_MESSAGE_ADD',
            'title': ''+top.subject,
            'href': 'https://mail.google.com/mail/u/0/#inbox/' + top.EmailID
        };
